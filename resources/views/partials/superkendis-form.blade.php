{{--
    Form Generate Superkendis per Pelaksana (di-share antara halaman
    requests/superkendis dan checklists/edit "Pengeluaran Riil").

    Variabel yang dibutuhkan:
    - $superkendisDone   : bool, checklist Surat Tugas sudah Lengkap
    - $stChecklist       : SpjChecklist (Surat Tugas) dengan relasi suratTugasDetail.pelaksanas.superkendis
    - $kecamatans        : Collection SkRatePerjalanan
    - $selectedPelaksanaIds : array id pelaksana yang dipilih via ?pelaksana=
--}}
<div id="superkendis-generate" class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
    ...
    @if($superkendisDone && $stChecklist && $stChecklist->suratTugasDetail && $stChecklist->suratTugasDetail->pelaksanas->count() > 0)
        <form method="POST" action="{{ route('requests.superkendis.bulk', $stChecklist->request_id) }}" id="superkendis-form">
            @csrf
            ...
            <div class="mt-6 flex items-center gap-2">
                ...
                <a href="{{ route('requests.superkendis', $stChecklist->request_id) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2 px-4 rounded text-sm">
                    Buka Halaman Superkendis
                </a>
            </div>
        </form>
    @else
