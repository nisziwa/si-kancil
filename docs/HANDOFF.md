...
- Dropdown POK pada modal Generate Laporan (`checklists/edit.blade.php`) kini **grouped per kegiatan**: header `kode_kegiatan` + `nama_kegiatan`, rincian di-indent (bukan list datar).
- Seluruh POK otomatis dimuat saat field difokuskan / modal dibuka (`/travel-reports/pok/search` tanpa `q`, tanpa limit).
- Filtering realtime berbasis `nama_rincian_pok`; grouping dipertahankan; kata kunci kosong menampilkan semua.
...
Status: Completed
- **Template HONOR** kini `KAK, FPA, Kuitansi BOS, Surat Tugas` (Surat Tugas ikut checklist kelengkapan dokumen).
- **Service terpusat** `App\Services\ChecklistStatusGate` (baru): satu sumber aturan validasi perpindahan checklist yang dipakai kanban single, kanban bulk, dan dropdown -> hasil validasi single = bulk. Pagar: ST dependen (`st_self_incomplete`, `st_dependent`), **Dokumentasi `Belum Lengkap` memblokir Laporan Perjalanan ke Lengkap/Perlu Perbaikan** (`dokumentasi_belum_lengkap`), pengumpulan laporan belum tuntas (`laporan_not_collected`), dan checklist ke Perlu Perbaikan saat SPJ `Dikirim ke PPK` (`spj_needs_perbaikan`).
- **Dokumentasi vs Laporan Perjalanan** (PERJADIN/TRANSLOK/TRANSLOK_DS): Dokumentasi `Belum Lengkap` -> Laporan maksimal `Belum Lengkap`; Dokumentasi `Lengkap`/`Perlu Perbaikan` -> Laporan bebas Lengkap/Perbaikan. Status keduanya tidak harus sama.
- **Checklist ke Perlu Perbaikan saat SPJ `Dikirim ke PPK`**:
  - Kanban single: popup **Konfirmasi** + [Batal] [Ubah Status SPJ] (422 `require_spj_confirm`); [Ubah Status SPJ] -> kirim ulang `confirm_spj=1` -> checklist `Perlu Perbaikan` + SPJ `Perbaikan` (+ RequestStatusHistory `Dikirim ke PPK -> Perbaikan`).
  - Dropdown & bulk: tanpa popup interaktif, keduanya otomatis mengubah SPJ -> `Perbaikan` (aturan sama).
- **Transisi SPJ**: `Perbaikan` hanya -> `Dikirim ke PPK` (tidak lagi ke Selesai); `Selesai` hanya dari `Dikirim ke PPK` (`RequestStatusService::TRANSITIONS`).
- **Testing**: `tests/Feature/FpaKanbanStatusValidationFlowTest.php` (11 kasus) + update `RequestStatusTest`. Seluruh **143 tests PASS (504 assertions)**; npm build & view:cache sukses.
- **Sisa `alert()`/`confirm()` di luar scope status (dibiarkan)**: delete FPA (`requests/show.blade.php:11`, `requests/index.blade.php:84`), delete template (`templates/index.blade.php:100`), delete SK Rate (`sk_rates/index.blade.php:67`), toggle aktif master POK (`master_pok/index.blade.php:116`), confirm tambah pelaksana massal ST (`checklists/edit.blade.php:551`), alert "Laporan berhasil digenerate" (`checklists/edit.blade.php:911`). Bukan bagian flow perpindahan status.
